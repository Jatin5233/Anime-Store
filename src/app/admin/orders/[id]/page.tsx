...
export default function AdminOrderDetailPage() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        ...
        if (data.success) {
          setOrder(data.order);
          setEditingStatus(data.order.orderStatus);
          setEditingPayment(data.order.paymentStatus);
        } else {
  ...
  const handleSavePayment = async () => {
    if (!editingPayment || !order) return;
    ...
    try {
      const res = await fetch(`/api/admin/orders/${orderId}`, {
        ...
        body: JSON.stringify({ paymentStatus: editingPayment }),
      });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-xl border border-cyan-500/20 p-6">
              ...
              <p className="text-gray-400 mt-2">
                Customer: <span className="text-white font-semibold">{order.user.email}</span>
              </p>
            ...
            {/* Order Items */}
            <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-xl border border-cyan-500/20 p-6">
              <h3 className="text-xl font-bold text-white mb-6">Order Items ({order.items.length})</h3>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-xl border border-cyan-500/20 p-6">
              ...
              <select
                value={editingStatus || 'processing'}
                onChange={(e) => setEditingStatus(e.target.value)}
                className="w-full bg-gray-800/50 border border-cyan-500/30 rounded-lg px-4 py-3 text-white focus:border-cyan-400 focus:outline-none mb-4"
              >
              ...
              <button
                ...
                disabled={editingStatus === order.orderStatus || saving}
                className="w-full py-3 bg-gradient-to-r from-cyan-500 to-purple-600 text-white rounded-lg font-bold hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all"
              >
            ...
            <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-xl border border-cyan-500/20 p-6">
              ...
              <button
                ...
                disabled={editingPayment === order.paymentStatus || saving}
                className="w-full py-3 bg-gradient-to-r from-cyan-500 to-purple-600 text-white rounded-lg font-bold hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all"
              >
                {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
            ...
            {/* Order Info */}
            <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 rounded-xl border border-cyan-500/20 p-6">
              <h3 className="text-lg font-bold text-white mb-4">Order Info</h3>
              <div className="space-y-3 text-sm">
                ...
                <div>
                  <p className="text-gray-400">Items Count</p>
